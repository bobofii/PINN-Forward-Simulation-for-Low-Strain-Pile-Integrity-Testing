...
Abrupt enlargement pile: intact-pile background field + three-subdomain scattering field
"""
...
RESULT_DIR = "Results_AbruptEnlargement_BackgroundScattering_NoClip"
...
D_NORMAL = 0.5
D_ENLARGEMENT = 0.7
X_ENLARGEMENT_CENTER = 6.25
ENLARGEMENT_LENGTH = 0.5
...
A_ENLARGEMENT = np.pi * (D_ENLARGEMENT / 2.0) ** 2
A1_STAR = 1.0
A2_STAR = A_ENLARGEMENT / A_NORMAL
A3_STAR = 1.0
...
X_ENLARGEMENT_CENTER_STAR = X_ENLARGEMENT_CENTER / L
ENLARGEMENT_LENGTH_STAR = ENLARGEMENT_LENGTH / L
X_ENLARGEMENT_START_STAR = X_ENLARGEMENT_CENTER_STAR - 0.5 * ENLARGEMENT_LENGTH_STAR
X_ENLARGEMENT_END_STAR = X_ENLARGEMENT_CENTER_STAR + 0.5 * ENLARGEMENT_LENGTH_STAR

T_ENLARGEMENT_START_ARRIVE_STAR = X_ENLARGEMENT_START_STAR
T_ENLARGEMENT_END_ARRIVE_STAR = X_ENLARGEMENT_END_STAR
T_ENLARGEMENT_START_REFLECT_STAR = 2.0 * X_ENLARGEMENT_START_STAR
T_ENLARGEMENT_END_REFLECT_STAR = 2.0 * X_ENLARGEMENT_END_STAR
T_BOTTOM_ARRIVE_STAR = 1.0
T_BOTTOM_REFLECT_STAR = 2.0

print("=" * 72)
print("突变扩颈桩：完整桩背景场 + 三子域散射场 PINN")
print(f"设备: {device}")
print(f"桩长: {L:.3f} m")
print(f"正常桩径: {D_NORMAL:.3f} m")
print(f"扩颈段直径: {D_ENLARGEMENT:.3f} m")
print(f"扩颈段范围: {X_ENLARGEMENT_START_STAR * L:.3f} ~ {X_ENLARGEMENT_END_STAR * L:.3f} m")
print(f"面积比 A_enlargement/A_normal = {A2_STAR:.6f}")
...
class BackgroundScatteringEnlargementPINN(nn.Module):
    def __init__(self, background_model):
        super().__init__()
        self.background = background_model
        self.net1 = ScatterSubNet(0.0, X_ENLARGEMENT_START_STAR)
        self.net2 = ScatterSubNet(X_ENLARGEMENT_START_STAR, X_ENLARGEMENT_END_STAR)
        self.net3 = ScatterSubNet(X_ENLARGEMENT_END_STAR, 1.0)
    ...
    def scatter_piecewise(self, x_star, t_star):
        ...
        m1 = x_star < X_ENLARGEMENT_START_STAR
        m2 = (x_star >= X_ENLARGEMENT_START_STAR) & (x_star <= X_ENLARGEMENT_END_STAR)
        m3 = x_star > X_ENLARGEMENT_END_STAR
    ...
    def bc_top_residual(self, t_star):
        # 背景场已经承担全部桩顶激励，因此散射场采用齐次 Neumann 边界。
        t = t_star.clone().detach().requires_grad_(True)
        x0 = torch.zeros_like(t).requires_grad_(True)
        s = self.net1(x0, t)
    ...
    def interface_residuals(self, t_start_star, t_end_star):
        ...
        # 界面 1：正常段 -> 扩颈段
        xs_l = torch.full_like(ts, X_ENLARGEMENT_START_STAR).requires_grad_(True)
        xs_r = torch.full_like(ts, X_ENLARGEMENT_START_STAR).requires_grad_(True)
        ...
        u0_x_s = self._background_ux(X_ENLARGEMENT_START_STAR, ts)
        ...
        # 界面 2：扩颈段 -> 正常段
        xe_l = torch.full_like(te, X_ENLARGEMENT_END_STAR).requires_grad_(True)
        xe_r = torch.full_like(te, X_ENLARGEMENT_END_STAR).requires_grad_(True)
        ...
        u0_x_e = self._background_ux(X_ENLARGEMENT_END_STAR, te)
...
def make_segment1_pde(n):
    ...
    pair_uniform = uniform_pair(n_uniform, 0.0, X_ENLARGEMENT_START_STAR)

    x_if = sample_x_near_boundary(
        n_interface, X_ENLARGEMENT_START_STAR, "left", width=0.018
    )
    t_if = sample_time_around(
        [T_ENLARGEMENT_START_ARRIVE_STAR, T_ENLARGEMENT_START_REFLECT_STAR],
        n_interface,
        half_width=0.08,
    )

    x_return = sample_x_interval(n_return, 0.0, X_ENLARGEMENT_START_STAR)
    t_return = sample_time_around(
        [
            T_ENLARGEMENT_START_REFLECT_STAR,
            T_ENLARGEMENT_END_REFLECT_STAR,
...
def make_segment2_pde(n):
    ...
    pair_uniform = uniform_pair(n_uniform, X_ENLARGEMENT_START_STAR, X_ENLARGEMENT_END_STAR)

    width = min(0.012, 0.30 * ENLARGEMENT_LENGTH_STAR)
    x_start = sample_x_near_boundary(
        n_start, X_ENLARGEMENT_START_STAR, "right", width=width
    )
    t_start = sample_time_around(
        [T_ENLARGEMENT_START_ARRIVE_STAR], n_start, half_width=0.08
    )

    x_end = sample_x_near_boundary(n_end, X_ENLARGEMENT_END_STAR, "left", width=width)
    t_end = sample_time_around([T_ENLARGEMENT_END_ARRIVE_STAR], n_end, half_width=0.08)
...
def make_segment3_pde(n):
    ...
    pair_uniform = uniform_pair(n_uniform, X_ENLARGEMENT_END_STAR, 1.0)

    x_if = sample_x_near_boundary(
        n_interface, X_ENLARGEMENT_END_STAR, "right", width=0.018
    )
    t_if = sample_time_around(
        [T_ENLARGEMENT_END_ARRIVE_STAR], n_interface, half_width=0.08
    )

    x_bottom = sample_x_interval(
        n_bottom, max(X_ENLARGEMENT_END_STAR, 0.97), 1.0
    )
...
def sample_scatter_ic(n_total):
    ...
    x1 = sample_x_interval(n1, 0.0, X_ENLARGEMENT_START_STAR)
    x2 = sample_x_interval(n2, X_ENLARGEMENT_START_STAR, X_ENLARGEMENT_END_STAR)
    x3 = sample_x_interval(n3, X_ENLARGEMENT_END_STAR, 1.0)

    # 只在原有 IC 损失内放入少量端点/界面锚点，不增加新损失。
    anchors1 = torch.tensor([[0.0], [X_ENLARGEMENT_START_STAR]], device=device)
    anchors2 = torch.tensor(
        [[X_ENLARGEMENT_START_STAR], [X_ENLARGEMENT_END_STAR]], device=device
    )
    anchors3 = torch.tensor([[X_ENLARGEMENT_END_STAR], [1.0]], device=device)
...
def sample_scatter_top(n_bc):
    ...
    return _shuffle_cat(
        ...
        sample_time_around(
            [T_ENLARGEMENT_START_REFLECT_STAR, T_ENLARGEMENT_END_REFLECT_STAR],
            n_defect,
            half_width=0.10,
        ),
        sample_time_around(
...
def make_scatter_batch(n_pde=14000, n_ic=1800, n_bc=1500, n_if=1800):
    ...
    return {
        ...
        "interface_start_t": sample_interface_time(
            n_if,
            T_ENLARGEMENT_START_ARRIVE_STAR,
            [T_ENLARGEMENT_START_REFLECT_STAR, T_ENLARGEMENT_END_REFLECT_STAR],
        ),
        "interface_end_t": sample_interface_time(
            n_if,
            T_ENLARGEMENT_END_ARRIVE_STAR,
            [T_ENLARGEMENT_START_REFLECT_STAR, T_ENLARGEMENT_END_REFLECT_STAR],
        ),
    }
...
def plot_loss_histories(model):
    ...
    plt.semilogy(epochs, h["pde_2"], label="Train PDE - enlargement segment")
    ...
    if h["val_epoch"]:
        ...
        plt.title("Validation loss history - abrupt enlargement pile")
        plt.grid(True, alpha=0.3)
        plt.legend(ncol=2)
        plt.tight_layout()
        plt.savefig(
...
def plot_top_velocity(model):
    ...
    plt.title("Pile-head velocity - abrupt enlargement pile")
    plt.xlim(0.0, T_OBS * 1e3)
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def plot_displacement_contour(X, T, u_star):
    ...
    plt.axhspan(
        X_ENLARGEMENT_START_STAR * L,
        X_ENLARGEMENT_END_STAR * L,
        alpha=0.18,
        label="Abrupt enlargement region",
    )
    plt.axhline(X_ENLARGEMENT_START_STAR * L, linestyle="--", linewidth=1.1)
    plt.axhline(X_ENLARGEMENT_END_STAR * L, linestyle="--", linewidth=1.1)
    plt.xlabel("Time t / ms")
    plt.ylabel("Pile position x / m")
    plt.title("Displacement contour - abrupt enlargement pile")
...
def plot_velocity_contour(X, T, u_t_star):
    ...
    plt.xlabel("Time t / ms")
    plt.ylabel("Pile position x / m")
    plt.title("Velocity contour - abrupt enlargement pile")
    plt.legend(loc="upper right")
    plt.tight_layout()
...
def save_top_velocity_csv(model):
    ...
    path = os.path.join(RESULT_DIR, "pinn_abrupt_enlargement_top_velocity.csv")
    np.savetxt(
        path,
        data,
        delimiter=",",
...
def find_abaqus_rpt():
    preferred_names = [
        "XY_pile_abrupt_enlargement_1D.rpt",
        "XY_pile_abrupt_enlargement_1D(2).rpt",
        "abaqus_abrupt_enlargement_top_velocity.rpt",
    ]
    ...
    # 最后再搜索文件名中包含 enlargement 的 rpt
    for directory in search_dirs:
        if not os.path.isdir(directory):
            continue
        for name in os.listdir(directory):
            lower = name.lower()
            if lower.endswith(".rpt") and ("enlargement" in lower or "扩颈" in lower):
...
def plot_abaqus_comparison(model):
    ...
    if rpt_path is None:
        print("未找到突变扩径桩 ABAQUS .rpt，跳过对比图。")
        return
    ...
    plt.title("Abrupt enlargement pile: ABAQUS vs background-scattering PINN")
    plt.xlim(0.0, T_OBS * 1e3)
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(RESULT_DIR, "abaqus_vs_pinn_abrupt_enlargement.png"), dpi=300)
    plt.close()
...
def save_scatter_model(model, background_path):
    path = os.path.join(RESULT_DIR, "pinn_abrupt_enlargement_background_scattering_model.pth")
    torch.save(
        {
            ...
            "parameters": {
                "L": L,
                "D_normal": D_NORMAL,
                "D_enlargement": D_ENLARGEMENT,
                "x_enlargement_center": X_ENLARGEMENT_CENTER,
                "enlargement_length": ENLARGEMENT_LENGTH,
                "rho0": RHO0,
                "E0": E0,
                "T_obs": T_OBS,
                "T_pulse": T_PULSE,
...
if __name__ == "__main__":
    ...
    model = BackgroundScatteringEnlargementPINN(background_model).to(device)
