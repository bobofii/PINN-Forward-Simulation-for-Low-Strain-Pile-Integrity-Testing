...
Abrupt segregation pile: intact-pile background field + three-subdomain scattering field
"""
...
RESULT_DIR = "Results_AbruptSegregation_BackgroundScattering_NoClip"
...
L = 13.0
D_NORMAL = 0.5
D_SEGREGATION = 0.5          # 离析段直径不变
X_SEGREGATION_CENTER = 6.25
SEGREGATION_LENGTH = 0.5
...
A_SEGREGATION = np.pi * (D_SEGREGATION / 2.0) ** 2
A1_STAR = 1.0
A2_STAR = A_SEGREGATION / A_NORMAL
A3_STAR = 1.0
...
# 无量纲轴向刚度 EA/(E0*A0)，用于界面轴力连续条件
EA1_STAR = E1_STAR * A1_STAR
EA2_STAR = E2_STAR * A2_STAR
EA3_STAR = E3_STAR * A3_STAR

C0 = np.sqrt(E0 / RHO0)
C2 = np.sqrt(E2 / RHO2)
C2_RATIO = C2 / C0
Z0 = A_NORMAL * np.sqrt(E0 * RHO0)
Z2 = A_SEGREGATION * np.sqrt(E2 * RHO2)
Z2_RATIO = Z2 / Z0
...
X_SEGREGATION_CENTER_STAR = X_SEGREGATION_CENTER / L
SEGREGATION_LENGTH_STAR = SEGREGATION_LENGTH / L
X_SEGREGATION_START_STAR = (
    X_SEGREGATION_CENTER_STAR - 0.5 * SEGREGATION_LENGTH_STAR
)
X_SEGREGATION_END_STAR = (
    X_SEGREGATION_CENTER_STAR + 0.5 * SEGREGATION_LENGTH_STAR
)


T_SEGREGATION_START_ARRIVE_STAR = X_SEGREGATION_START_STAR
T_SEGREGATION_TRANSIT_STAR = SEGREGATION_LENGTH_STAR / C2_RATIO
T_SEGREGATION_END_ARRIVE_STAR = (
    X_SEGREGATION_START_STAR + T_SEGREGATION_TRANSIT_STAR
)
T_SEGREGATION_START_REFLECT_STAR = 2.0 * X_SEGREGATION_START_STAR
T_SEGREGATION_END_REFLECT_STAR = (
    2.0 * X_SEGREGATION_START_STAR + 2.0 * T_SEGREGATION_TRANSIT_STAR
)
T_BOTTOM_ARRIVE_STAR = (
    X_SEGREGATION_START_STAR
    + T_SEGREGATION_TRANSIT_STAR
    + (1.0 - X_SEGREGATION_END_STAR)
)
T_BOTTOM_REFLECT_STAR = 2.0 * T_BOTTOM_ARRIVE_STAR

print("=" * 72)
print("突变离析桩：完整桩背景场 + 三子域散射场 PINN")
print(f"设备: {device}")
print(f"桩长: {L:.3f} m")
print(f"正常段/离析段直径: {D_NORMAL:.3f} m / {D_SEGREGATION:.3f} m")
print(
    f"离析段范围: {X_SEGREGATION_START_STAR * L:.3f} ~ "
    f"{X_SEGREGATION_END_STAR * L:.3f} m"
)
print(f"面积系数 A2* = {A2_STAR:.6f}")
print(f"弹性模量系数 E2* = {E2_STAR:.6f}")
print(f"密度系数 rho2* = {RHO2_STAR:.6f}")
print(f"正常段波速 c0 = {C0:.3f} m/s")
print(f"离析段波速 c2 = {C2:.3f} m/s, c2/c0 = {C2_RATIO:.6f}")
print(f"波阻抗比 Z2/Z0 = {Z2_RATIO:.6f}")
...
class BackgroundScatteringSegregationPINN(nn.Module):
    def __init__(self, background_model):
        super().__init__()
        self.background = background_model
        self.net1 = ScatterSubNet(0.0, X_SEGREGATION_START_STAR)
        self.net2 = ScatterSubNet(X_SEGREGATION_START_STAR, X_SEGREGATION_END_STAR)
        self.net3 = ScatterSubNet(X_SEGREGATION_END_STAR, 1.0)
    ...
    def scatter_piecewise(self, x_star, t_star):
        ...
        m1 = x_star < X_SEGREGATION_START_STAR
        m2 = (x_star >= X_SEGREGATION_START_STAR) & (x_star <= X_SEGREGATION_END_STAR)
        m3 = x_star > X_SEGREGATION_END_STAR
    ...
    @staticmethod
    def _pde_normal_segment(net, x_star, t_star):
        """正常材料段的齐次散射方程：s_tt - s_xx = 0。"""
        x = x_star.clone().detach().requires_grad_(True)
        t = t_star.clone().detach().requires_grad_(True)
        s = net(x, t)
    ...
    def _pde_segregation_segment(self, x_star, t_star):
        x = x_star.clone().detach().requires_grad_(True)
        t = t_star.clone().detach().requires_grad_(True)

        s = self.net2(x, t)
        s_x = grad(
            s, x, torch.ones_like(s), create_graph=True, retain_graph=True
        )[0]
        s_t = grad(
            s, t, torch.ones_like(s), create_graph=True, retain_graph=True
        )[0]
        s_xx = grad(
            s_x, x, torch.ones_like(s_x), create_graph=True, retain_graph=True
        )[0]
        s_tt = grad(
            s_t, t, torch.ones_like(s_t), create_graph=True, retain_graph=True
        )[0]

        u0 = self.background(x, t)
        u0_x = grad(
            u0, x, torch.ones_like(u0), create_graph=True, retain_graph=True
        )[0]
        u0_t = grad(
            u0, t, torch.ones_like(u0), create_graph=True, retain_graph=True
        )[0]
        u0_xx = grad(
            u0_x, x, torch.ones_like(u0_x), create_graph=False,
            retain_graph=True
        )[0].detach()
        u0_tt = grad(
            u0_t, t, torch.ones_like(u0_t), create_graph=False
        )[0].detach()

        return (
            RHO2_STAR * (u0_tt + s_tt)
            - E2_STAR * (u0_xx + s_xx)
        )

    def pde_residuals(self, batch):
        return (
            self._pde_normal_segment(self.net1, *batch["pde_1"]),
            self._pde_segregation_segment(*batch["pde_2"]),
            self._pde_normal_segment(self.net3, *batch["pde_3"]),
        )
    ...
    def interface_residuals(self, t_start_star, t_end_star):
        ...
        # 界面 1：正常材料段 -> 离析材料段
        xs_l = torch.full_like(ts, X_SEGREGATION_START_STAR).requires_grad_(True)
        xs_r = torch.full_like(ts, X_SEGREGATION_START_STAR).requires_grad_(True)
        ...
        u0_x_s = self._background_ux(X_SEGREGATION_START_STAR, ts)
        ...
        r_f_s = (
            EA1_STAR * s1_x
            - EA2_STAR * s2s_x
            + (EA1_STAR - EA2_STAR) * u0_x_s
        )

        # 界面 2：离析材料段 -> 正常材料段
        xe_l = torch.full_like(te, X_SEGREGATION_END_STAR).requires_grad_(True)
        xe_r = torch.full_like(te, X_SEGREGATION_END_STAR).requires_grad_(True)
        ...
        u0_x_e = self._background_ux(X_SEGREGATION_END_STAR, te)
    ...
    def loss(self, batch, weights, record=False, epoch=None):
        ...
        pde_w1 = weights.get("pde_1", 0.35)
        pde_w2 = weights.get("pde_2", 0.30)
        pde_w3 = weights.get("pde_3", 0.35)
        l_pde = (
        ...
        l_interface = (
            ...
            + weights.get("interface_force", 2.5) * l_if_f
        )
...
def make_segment1_pde(n):
    ...
    pair_uniform = uniform_pair(n_uniform, 0.0, X_SEGREGATION_START_STAR)

    x_if = sample_x_near_boundary(
        n_interface, X_SEGREGATION_START_STAR, "left", width=0.018
    )
    t_if = sample_time_around(
        [T_SEGREGATION_START_ARRIVE_STAR, T_SEGREGATION_START_REFLECT_STAR],
        n_interface,
        half_width=0.08,
    )

    x_return = sample_x_interval(n_return, 0.0, X_SEGREGATION_START_STAR)
    t_return = sample_time_around(
        [
            T_SEGREGATION_START_REFLECT_STAR,
            T_SEGREGATION_END_REFLECT_STAR,
...
def make_segment2_pde(n):
    ...
    pair_uniform = uniform_pair(n_uniform, X_SEGREGATION_START_STAR, X_SEGREGATION_END_STAR)

    width = min(0.012, 0.30 * SEGREGATION_LENGTH_STAR)
    x_start = sample_x_near_boundary(
        n_start, X_SEGREGATION_START_STAR, "right", width=width
    )
    t_start = sample_time_around(
        [T_SEGREGATION_START_ARRIVE_STAR], n_start, half_width=0.08
    )

    x_end = sample_x_near_boundary(n_end, X_SEGREGATION_END_STAR, "left", width=width)
    t_end = sample_time_around([T_SEGREGATION_END_ARRIVE_STAR], n_end, half_width=0.08)
...
def make_segment3_pde(n):
    ...
    pair_uniform = uniform_pair(n_uniform, X_SEGREGATION_END_STAR, 1.0)

    x_if = sample_x_near_boundary(
        n_interface, X_SEGREGATION_END_STAR, "right", width=0.018
    )
    t_if = sample_time_around(
        [T_SEGREGATION_END_ARRIVE_STAR], n_interface, half_width=0.08
    )

    x_bottom = sample_x_interval(
        n_bottom, max(X_SEGREGATION_END_STAR, 0.97), 1.0
    )
...
def sample_scatter_ic(n_total):
    ...
    x1 = sample_x_interval(n1, 0.0, X_SEGREGATION_START_STAR)
    x2 = sample_x_interval(n2, X_SEGREGATION_START_STAR, X_SEGREGATION_END_STAR)
    x3 = sample_x_interval(n3, X_SEGREGATION_END_STAR, 1.0)


    anchors1 = torch.tensor([[0.0], [X_SEGREGATION_START_STAR]], device=device)
    anchors2 = torch.tensor(
        [[X_SEGREGATION_START_STAR], [X_SEGREGATION_END_STAR]], device=device
    )
    anchors3 = torch.tensor([[X_SEGREGATION_END_STAR], [1.0]], device=device)
...
def sample_scatter_top(n_bc):
    ...
    return _shuffle_cat(
        ...
        sample_time_around(
            [T_SEGREGATION_START_REFLECT_STAR, T_SEGREGATION_END_REFLECT_STAR],
            n_defect,
            half_width=0.10,
        ),
        sample_time_around(
...
def make_scatter_batch(n_pde=14000, n_ic=1800, n_bc=1500, n_if=1800):
    ...
    return {
        ...
        "interface_start_t": sample_interface_time(
            n_if,
            T_SEGREGATION_START_ARRIVE_STAR,
            [T_SEGREGATION_START_REFLECT_STAR, T_SEGREGATION_END_REFLECT_STAR],
        ),
...
def train_scatter_model(model):
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"A2_star={A2_STAR:.12e}\n")
        f.write(f"E2_star={E2_STAR:.12e}\n")
        f.write(f"rho2_star={RHO2_STAR:.12e}\n")
        f.write(f"c2_over_c0={C2_RATIO:.12e}\n")
        f.write(f"Z2_over_Z0={Z2_RATIO:.12e}\n")
...
def plot_loss_histories(model):
    ...
    plt.semilogy(epochs, h["pde_2"], label="Train PDE - segregation material segment")
    ...
    if h["val_epoch"]:
        ...
        plt.title("Validation loss history - abrupt segregation pile")
        plt.grid(True, alpha=0.3)
        plt.legend(ncol=2)
        plt.tight_layout()
        plt.savefig(
...
def plot_top_velocity(model):
    ...
    plt.title("Pile-head velocity - abrupt segregation pile")
    plt.xlim(0.0, T_OBS * 1e3)
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def plot_displacement_contour(X, T, u_star):
    ...
    plt.axhspan(
        X_SEGREGATION_START_STAR * L,
        X_SEGREGATION_END_STAR * L,
        alpha=0.18,
        label="Abrupt segregation region",
    )
    plt.axhline(X_SEGREGATION_START_STAR * L, linestyle="--", linewidth=1.1)
    plt.axhline(X_SEGREGATION_END_STAR * L, linestyle="--", linewidth=1.1)
    plt.xlabel("Time t / ms")
    plt.ylabel("Pile position x / m")
    plt.title("Displacement contour - abrupt segregation pile")
...
def plot_velocity_contour(X, T, u_t_star):
    ...
    plt.xlabel("Time t / ms")
    plt.ylabel("Pile position x / m")
    plt.title("Velocity contour - abrupt segregation pile")
    plt.legend(loc="upper right")
    plt.tight_layout()
...
def save_top_velocity_csv(model):
    ...
    path = os.path.join(RESULT_DIR, "pinn_abrupt_segregation_top_velocity.csv")
    np.savetxt(
        path,
        data,
        delimiter=",",
...
def find_abaqus_rpt():
    preferred_names = [
        "XY_pile_abrupt_segregation_1D.rpt",
        "XY_pile_abrupt_segregation_1D(1).rpt",
        "XY_pile_abrupt_segregate_1D.rpt",
        "XY_pile_abrupt_segregate_1D(1).rpt",
        "abaqus_abrupt_segregation_top_velocity.rpt",
    ]
    ...
    for directory in search_dirs:
        ...
        for name in os.listdir(directory):
            lower = name.lower()
            if lower.endswith(".rpt") and (
                "segregation" in lower
                or "segregate" in lower
                or "离析" in name
            ):
...
def plot_abaqus_comparison(model):
    ...
    if rpt_path is None:
        print("未找到突变离析 ABAQUS .rpt，跳过对比图。")
        return
    ...
    plt.title("Abrupt segregation pile: ABAQUS vs background-scattering PINN")
    plt.xlim(0.0, T_OBS * 1e3)
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(RESULT_DIR, "abaqus_vs_pinn_abrupt_segregation.png"), dpi=300)
    plt.close()
...
def save_scatter_model(model, background_path):
    path = os.path.join(RESULT_DIR, "pinn_abrupt_segregation_background_scattering_model.pth")
    torch.save(
        {
            ...
            "parameters": {
                "L": L,
                "D_normal": D_NORMAL,
                "D_segregation": D_SEGREGATION,
                "x_segregation_center": X_SEGREGATION_CENTER,
                "segregation_length": SEGREGATION_LENGTH,
                "rho0": RHO0,
                "rho2": RHO2,
                "E0": E0,
                "E2": E2,
...
if __name__ == "__main__":
    ...
    model = BackgroundScatteringSegregationPINN(background_model).to(device)
